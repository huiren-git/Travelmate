...
async def find_poi(city: str, name: str) -> dict[str, Any] | None:
    ...
    return next((poi for poi in pois if poi.get("name") == name), None)
...
async def adapt_reference_trip(reference: dict[str, Any], *, duration: int, start_date: str, travelers: int) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for activity in sequence:
        ...
        adapted.append((replacement or {}).get("name") or activity)
    sequence = adapted
    ...
    for index, activity in enumerate(sequence):
        day = index % duration
        hours = _duration(rhythm[index] if index < len(rhythm) else "1h")
        start = current_minutes[day]
        days[day]["items"].append({"time": f"{start // 60:02d}:{start % 60:02d}", "activity": activity, "duration": f"{hours}h", "status": "upcoming", "address": None, "image_url": "", "tips": None})
    ...
    return {"draft_daily_itinerary": days, "draft_budget": budget, "weather_info": weather, "adaptation_log": log}, log
