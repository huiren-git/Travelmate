          ...
          <div key={message.id} className={['flex gap-3', isUser ? 'justify-end' : 'justify-start'].join(' ')}>
            ...
            <div
              className={[
                ...
                isUser ? 'text-white' : 'bg-white text-slate-800 ring-1 ring-slate-100',
              ].join(' ')}
            ...
            >
              ...
              <div className={['mt-1 text-[11px]', isUser ? 'text-white/80' : 'text-slate-400'].join(' ')}>
                {message.time}
              </div>
            </div>
            {isUser && <Avatar size={36} className="bg-slate-200 text-slate-700 shrink-0" icon={<UserOutlined />} />}
          </div>
        )
      })}
    </div>
