...
export function TravelLogisticsCard({ logistics, onConfirm }: { logistics?: TravelLogistics; onConfirm?: (key: string) => void }) {
  ...
  return <Card size="small" title="交通与住宿" className="rounded-2xl shadow-sm">
    <div className="flex justify-between gap-2"><div><div className="font-semibold">全程住宿：{logistics.accommodation.area}</div><div className="mt-1 text-slate-500">{logistics.accommodation.nights} 晚 · {logistics.accommodation.rooms} 间 · 规则估算 ¥{logistics.accommodation.cost}</div></div>{logistics.accommodation.status === 'confirmed' ? <Tag color="green">已确认</Tag> : <Button size="small" onClick={() => onConfirm?.('accommodation')}>确认方案</Button>}</div>
